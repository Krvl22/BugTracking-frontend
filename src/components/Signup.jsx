...
// ✅ Admin removed from signup — admin is created by system only
const ROLES = [
  { value: "developer",       label: "Developer" },
  { value: "tester",          label: "Tester" },
  { value: "project_manager", label: "Project Manager" },
...
const Signup = () => {
  ...
  const {
    ...
    // ✅ Only validate on submit/blur, NOT on page load
    mode: "onTouched",
  });

  const [show, setShow]       = useState({ password: false, confirm: false });
  const [loading, setLoading] = useState(false);
  // ✅ Track if form was submitted once (for terms error placement)
      ...
      <div className="relative w-full max-w-lg">
        <div className="backdrop-blur-xl bg-white/10 rounded-2xl shadow-2xl border border-white/20 p-8 my-8">
          ...
          <form onSubmit={handleSubmit(submitHandler)} className="space-y-5">
            ...
            {/* ✅ Password — masked by default, with label */}
            <div>
              <label className="block text-sm font-medium text-white mb-2">
                Password
              </label>
            ...
            {/* ✅ Confirm Password — masked by default, with label */}
            <div>
              <label className="block text-sm font-medium text-white mb-2">
                Confirm Password
              </label>
            ...
            {/* ✅ Terms — error shown INSIDE the form, directly below checkbox */}
            <div>
              <div className="flex items-start space-x-3">
                <input
                  type="checkbox"
              ...
              {/* ✅ Terms error is inside the form flow, below the checkbox */}
              {submitted && !terms && (
                <p className="text-red-400 text-xs mt-2 flex items-center gap-1 ml-7">
                  <span>⚠</span> You must accept the Terms & Conditions to continue
                </p>
              )}
            </div>
