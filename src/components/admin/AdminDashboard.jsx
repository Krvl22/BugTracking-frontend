...
const AdminDashboard = () => {
  ...
  // ── Step 1: Replace hardcoded data with state ──
  const [stats, setStats] = useState([
    { label: 'Total Users',     value: '...', change: '+12%', icon: '👥', color: 'from-blue-500 to-cyan-500' },
    { label: 'Active Projects', value: '...', change: '+5%',  icon: '📊', color: 'from-emerald-500 to-teal-500' },
    { label: 'Total Bugs',      value: '...', change: '-8%',  icon: '🐛', color: 'from-orange-500 to-red-500' },
    { label: 'System Health',   value: '...', change: '+2%',  icon: '⚡', color: 'from-purple-500 to-pink-500' },
  ]);
  const [recentUsers, setRecentUsers] = useState([]);
  const [projects, setProjects]       = useState([]);
  const [loading, setLoading]         = useState(true);

  // ── Step 2: Fetch all data from backend on page load ──
  useEffect(() => {
    // Get token saved during login
    const token = localStorage.getItem("token");

    // This header is sent with every request so backend knows who you are
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const statsRes  = await fetch("http://localhost:3000/admin/stats", { headers });
        const statsData = await statsRes.json();

        if (statsData.success) {
          // Map backend response to the shape the UI expects
          setStats([
            { label: 'Total Users',     value: String(statsData.data.totalUsers),    change: '+12%', icon: '👥', color: 'from-blue-500 to-cyan-500' },
            { label: 'Active Projects', value: String(statsData.data.totalProjects), change: '+5%',  icon: '📊', color: 'from-emerald-500 to-teal-500' },
            { label: 'Total Bugs',      value: String(statsData.data.totalBugs),     change: '-8%',  icon: '🐛', color: 'from-orange-500 to-red-500' },
            { label: 'System Health',   value: statsData.data.systemHealth,          change: '+2%',  icon: '⚡', color: 'from-purple-500 to-pink-500' },
          ]);
        }

        // ── Fetch Recent Users (/admin/recent-users) ──
        const usersRes  = await fetch("http://localhost:3000/admin/recent-users", { headers });
        ...
        // ── Fetch Projects (/admin/projects) ──
        const projRes  = await fetch("http://localhost:3000/admin/projects", { headers });
        const projData = await projRes.json();
        console.log("Projects response:", projData); // ← add this temporarily to check
                ...
                <div className="space-y-4">
                  {recentUsers.map((user, index) => (
                    <div key={index} className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/10 hover:bg-white/10 transition-all duration-200">
                      <div className="flex items-center space-x-4">
                        <div className="w-10 h-10 rounded-full bg-linear-to-r from-blue-500 to-cyan-500 flex items-center justify-center text-white font-bold">
                          {/* ── FIXED: backend sends firstName, not name ── */}
                          {user.firstName?.charAt(0) || "U"}
                        </div>
                        <div>
                          {/* ── FIXED: combine firstName + lastName ── */}
                      ...
                      <div className="text-right">
                        <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${
                          ...
                          {/* ── FIXED: backend sends "active" lowercase ── */}
                          {user.status}
                        </span>
                        <p className="text-slate-400 text-xs mt-1">
                          {/* ── FIXED: backend sends createdAt date, not "joined" string ── */}
                  ...
                  <div key={index} className="bg-white/5 rounded-xl p-4 border border-white/10 hover:bg-white/10 transition-all duration-200">
                    ...
                    <div className="space-y-2 mb-4">
                      <div className="flex justify-between text-sm">
                        ...
                        {/* ── FIXED: backend has no "team" field, using projectKey instead ── */}
                        <span className="text-white">{project.projectKey}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                    ...
                    <div className="mb-2">
                      ...
                      {/* Progress bar kept for UI — shows 100% for active, 50% for inactive */}
                      <div className="w-full h-2 bg-white/10 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-linear-to-r from-blue-500 to-cyan-500"
                          style={{ width: project.status === 'active' ? '75%' : '40%' }}
                        ></div>
                      </div>
                    </div>
                    <span className="inline-block px-3 py-1 rounded-full text-xs font-medium bg-blue-500/20 text-blue-400">
