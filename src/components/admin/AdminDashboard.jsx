...
//   // ✅ Sync currentUser from localStorage — updates when settings changes profile
//   const [currentUser, setCurrentUser] = useState(() =>
//     JSON.parse(localStorage.getItem('user') || '{}')
//   )
//   useEffect(() => {
...
//       {/* Sidebar */}
//       <aside className={`fixed top-0 left-0 z-40 w-64 h-screen transition-transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
//         <div className="h-full px-3 py-4 overflow-y-auto no-scrollbar backdrop-blur-xl bg-white/10 border-r border-white/20 flex flex-col">
//           <div className="flex items-center justify-between mb-8 px-3">
...
//                   : <div className="w-10 h-10 rounded-full bg-linear-to-r from-blue-500 to-cyan-500 flex items-center justify-center text-white font-bold shrink-0">{currentUser.firstName?.charAt(0) || 'A'}</div>
//                 }
//                 <div className="min-w-0 flex-1">
//                   <p className="text-white text-sm font-medium truncate">{currentUser.firstName} {currentUser.lastName}</p>
//                   <p className="text-slate-400 text-xs truncate">{currentUser.email}</p>
...
//             <div>
//               <h1 className="text-2xl font-bold text-white">Admin Dashboard</h1>
//               {/* ✅ Uses actual first name */}
//               <p className="text-slate-300 text-sm">Welcome back, {currentUser.firstName || 'Admin'}!</p>
//             </div>
//           </div>
//           <div className="flex items-center space-x-3">
//             {/* ✅ NotificationBell — self-contained, fetches its own data */}
//             <NotificationBell />
//             <button onClick={handleLogout} className="px-4 py-2 bg-linear-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white rounded-lg text-sm font-medium transition-all">
//               Logout
//             </button>
...
//                           : <div className="w-10 h-10 rounded-full bg-linear-to-r from-blue-500 to-cyan-500 flex items-center justify-center text-white font-bold">{u.firstName?.charAt(0) || 'U'}</div>
//                         }
//                         <div>
//                           <p className="text-white font-medium">{u.firstName} {u.lastName}</p>
//                           <p className="text-slate-400 text-sm">{u.email}</p>
...
//           {/* ✅ Active Projects only — completed projects no longer show here */}
//           <div className="backdrop-blur-xl bg-white/10 rounded-2xl p-6 border border-white/20">
//             <div className="flex items-center justify-between mb-6">
...
//                       <div className="h-full bg-linear-to-r from-blue-500 to-cyan-500 rounded-full" style={{ width: '60%' }} />
//                     </div>
