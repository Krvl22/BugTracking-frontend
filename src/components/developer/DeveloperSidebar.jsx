...
// //   const [counts, setCounts] = useState({ tasks: 0, bugs: 0, projects: 0 })
...
// //       fetch(`http://localhost:3000/developer/tasks?userId=${u._id}`,    { headers: h }),
// //       fetch(`http://localhost:3000/developer/bugs?userId=${u._id}`,     { headers: h }),
// //       fetch(`http://localhost:3000/developer/projects?userId=${u._id}`, { headers: h }),
// //     ])
// //     const [tData, bData, pData] = await Promise.all([tRes.json(), bRes.json(), pRes.json()])
// //     setCounts({
// //       tasks:    tData.success ? (tData.data || []).filter(t => !['completed'].includes(t.status)).length : 0,
// //       bugs:     bData.success ? (bData.data || []).filter(b => !b.resolved).length : 0,
// //       projects: pData.success ? (pData.data || []).filter(p => p.status === 'active').length : 0,
// //     })
...
// //   const NAV_ITEMS = [
// //     { name: 'Dashboard', path: '/developerdashboard', count: null,         icon: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6' },
// //     { name: 'My Tasks',  path: '/developer/tasks',    count: counts.tasks, icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4' },
// //     { name: 'My Bugs',   path: '/developer/bugs',     count: counts.bugs,  icon: 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z' },
// //     { name: 'Projects', path: '/developer/projects', count: counts.projects, icon: 'M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z' },
// //     { name: 'Analytics', path: '/developer/analytics', count: null, icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' },
// //     { name: 'Settings',  path: '/developer/settings', count: null,         icon: 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z' },
// //   ]
...
// //           <div className="flex items-center justify-between mb-8 px-3">
// //             <Link to="/developerdashboard" className="text-xl font-bold text-white hover:text-cyan-300 transition-colors">Bug Tracker</Link>
// //             <button onClick={() => setSidebarOpen(false)} className="lg:hidden text-white">
// //               <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
// //                 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
// //               </svg>
...
// //             <Link to="/developer/settings" className="block bg-white/5 rounded-xl p-3 border border-white/10 hover:bg-white/10 transition-all group">
// //               <div className="flex items-center space-x-3">
// //                 {user.profilePic ? (
...
//   const [user, setUser]       = useState(() => JSON.parse(localStorage.getItem('user') || '{}'))
//   const [counts, setCounts]   = useState({ tasks: 0, bugs: 0, projects: 0 })
//   const [collapsed, setCollapsed] = useState(() => localStorage.getItem('devSidebarCollapsed') === 'true')

//   const toggleCollapse = () => {
//     setCollapsed(prev => {
//       const next = !prev
//       localStorage.setItem('devSidebarCollapsed', String(next))
...
//     { name: 'Projects',  path: '/developer/projects', count: counts.projects, icon: 'M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z' },
//     { name: 'Analytics', path: '/developer/analytics', count: null,          icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' },
//     { name: 'Settings',  path: '/developer/settings', count: null,           isSettings: true },
//   ]
...
//               <Link to="/developer/settings" title="Settings">
//                 {user.profilePic ? (
...
const DeveloperSidebar = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  const toggleCollapse = () => setCollapsed(prev => {
    const next = !prev
    localStorage.setItem('devSidebarCollapsed', String(next))
    window.dispatchEvent(new CustomEvent('sidebarToggled', { detail: { collapsed: next, role: 'developer' } }))
    return next
  })
  ...
  useEffect(() => {
    ...
    const fetchCounts = async () => {
      try {
        ...
        const [tData, bData, pData] = await Promise.all([tRes.json(), bRes.json(), pRes.json()])
        setCounts({
          tasks: tData.success ? (tData.data || []).filter(t => t.status !== 'completed').length : 0,
          bugs: bData.success ? (bData.data || []).filter(b => !b.resolved).length : 0,
          projects: pData.success ? (pData.data || []).filter(p => p.status === 'active').length : 0,
        })
      } catch (err) { console.error(err) }
  ...
  const NAV_ITEMS = [
    { name: 'Dashboard', path: '/developerdashboard',  countKey: null,          icon: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6' },
    { name: 'My Tasks',  path: '/developer/tasks',     countKey: 'tasks',       icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4' },
    { name: 'My Bugs',   path: '/developer/bugs',      countKey: 'bugs',        icon: 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z' },
    { name: 'Projects',  path: '/developer/projects',  countKey: 'projects',    icon: 'M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z' },
    { name: 'Analytics', path: '/developer/analytics', countKey: null,          icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' },
    { name: 'Settings',  path: '/developer/settings',  countKey: null,          isSettings: true },
  ]
      ...
      <aside className={`fixed top-0 left-0 z-40 h-screen transition-all duration-300 w-64 ${collapsed ? 'lg:w-16' : 'lg:w-64'} ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
        <div className="h-full flex flex-col px-3 py-4 overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden backdrop-blur-xl bg-slate-900/80 border-r border-white/10">
          ...
          <div className={`flex items-center mb-6 ${collapsed ? 'justify-center' : 'justify-between px-1'}`}>
            {!collapsed && (
              <div>
                <Link to="/developerdashboard" className="text-lg font-bold text-white hover:text-cyan-300 transition-colors">Bug Tracker</Link>
          ...
          <div className="mt-2 pt-2 border-t border-white/8">
            {!collapsed ? (
              <Link to="/developer/settings" className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-white/8 transition-all group">
                {user.profilePic ? (
