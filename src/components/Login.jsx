...
const Login = () => {
  ...
  // ✅ react-hook-form replaces all your useState field tracking
  ...
  const [showPassword, setShowPassword] = useState(false);
  const [loading, setLoading]           = useState(false);
  const selectedRole = watch("role"); // ✅ watch role for UI highlight
  ...
  // ✅ This is your submitHandler — same as mentor's pattern
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-900 via-blue-900 to-slate-900 flex items-center justify-center p-6">
      ...
      <div className="relative w-full max-w-md">
        <div className="backdrop-blur-xl bg-white/10 rounded-2xl shadow-2xl border border-white/20 p-8">
          ...
          {/* ✅ handleSubmit wraps your submitHandler — mentor's pattern */}
          <form onSubmit={handleSubmit(submitHandler)} className="space-y-6">

            {/* Role Selector — uses setValue since it's a custom button UI */}
            <div>
              <label className="block text-sm font-medium text-white mb-3">
                Select Your Role
              </label>
              <div className="grid grid-cols-2 gap-3">
                {ROLES.map(({ value, label }) => (
                  <button
                    key={value}
                    type="button"
                    onClick={() => setValue("role", value)} // ✅ setValue from hook-form
                    className={`py-3 px-4 rounded-xl text-sm font-medium transition-all ${
                      selectedRole === value
                        ? "bg-linear-to-r from-blue-500 to-cyan-500 text-white shadow-lg"
                        : "bg-white/5 text-slate-300 border border-white/10 hover:bg-white/10"
              ...
              {/* hidden input so role is included in form data */}
            ...
            <div>
              <input
                ...
                className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white"
                {...register("email", {
                  required: "Email is required" // ✅ mentor's pattern
                })}
              />
              {errors.email && (
            ...
            <div>
              <div className="relative">
                <input
                  ...
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white pr-12"
                  {...register("password", {
                    required: "Password is required", // ✅ mentor's pattern
