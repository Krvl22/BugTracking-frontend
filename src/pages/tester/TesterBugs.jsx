...
const TesterBugs = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res    = await fetch(`http://localhost:3000/tester/bugs?userId=${user._id}`, { headers: { Authorization: `Bearer ${token}` } })
        const result = await res.json()
        if (result.success) setData(result.data || [])
      } catch (err) { console.error(err) }
      finally { setLoading(false) }
      ...
      <div className="lg:ml-64">
        <header className="backdrop-blur-xl bg-white/10 border-b border-white/20 sticky top-0 z-30 px-4 py-4 lg:px-8 flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div><h1 className="text-2xl font-bold text-white">My Bugs</h1><p className="text-slate-300 text-sm">Bugs you have reported</p></div>
          </div>
                ...
                {filtered.map(bug => (
                  <div key={bug._id}
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-3">
                      ...
                      {bug.attachmentUrl && <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-blue-500/20 text-blue-400">📎 Attachment</span>}
                    </div>
                    ...
                    <div className="flex justify-between items-center text-xs text-slate-400">
                      ...
                      <span className="text-blue-400 group-hover:text-blue-300 shrink-0">View Details →</span>
                    </div>
                  </div>
                ))}
              </div>
