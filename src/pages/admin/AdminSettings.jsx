...
const AdminSettings = () => {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      // Uses forgot-password → sends reset email
        ...
        <main className="p-4 lg:p-8 relative z-10 space-y-6 max-w-2xl">
          ...
          <div className="backdrop-blur-xl bg-white/10 rounded-2xl p-6 border border-white/20">
            ...
            <div className="flex items-center space-x-4 mb-6">
              <div className="w-16 h-16 rounded-full bg-linear-to-r from-blue-500 to-cyan-500 flex items-center justify-center text-white text-2xl font-bold">
                {profile.firstName?.charAt(0) || 'A'}
              </div>
              <div>
                <p className="text-white font-medium">{profile.firstName} {profile.lastName}</p>
                <p className="text-slate-400 text-sm">{profile.email}</p>
                <span className="text-xs px-2 py-1 rounded-full bg-blue-500/20 text-blue-400 mt-1 inline-block">
                  {profile.role}
                </span>
              </div>
            </div>
