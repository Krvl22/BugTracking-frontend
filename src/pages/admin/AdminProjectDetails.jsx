...
const AdminProjectDetails = () => {
  ...
  const fetchAll = async () => {
    try {
      const [projRes, modRes, taskRes, usersRes] = await Promise.all([
      ...
      const [projData, modData, taskData, usersData] = await Promise.all([
        projRes.json(), modRes.json(), taskRes.json(), usersRes.json()
      ])
  ...
  const handleAddTask = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (data.success) { setShowAddTask(false); setTaskForm({ title: '', description: '', priority: 'medium', moduleId: '', assignedTo: '', dueDate: '' }); fetchAll() }
  ...
  const taskStatusColor = (s) => ({
    ...
    bug_found: 'bg-red-500/20 text-red-400', fix_in_progress: 'bg-orange-500/20 text-orange-400',
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-900 via-blue-900 to-slate-900">
          ...
          <nav className="space-y-2 flex-1">
            {navItems.map(item => (
              <Link key={item.name} to={item.path}
                className={`flex items-center px-3 py-2.5 rounded-lg transition-all ${location.pathname === item.path ? 'bg-linear-to-r from-blue-500 to-cyan-500 text-white shadow-lg' : 'text-slate-300 hover:bg-white/10'}`}>
      ...
      <div className="lg:ml-64">
        <header className="backdrop-blur-xl bg-white/10 border-b border-white/20 sticky top-0 z-30 px-4 py-4 lg:px-8 flex items-center justify-between">
          ...
          <button onClick={handleLogout} className="px-4 py-2 bg-linear-to-r from-blue-500 to-cyan-500 text-white rounded-lg text-sm font-medium">Logout</button>
        </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="lg:col-span-2 grid grid-cols-2 gap-4">
                {[
                  ...
                  { label: 'In Progress',value: activeTasks,     color: 'text-yellow-400' },
                  { label: 'Bugs Found', value: bugTasks,        color: 'text-red-400' },
                ].map((s, i) => (
            ...
            {tasks.length > 0 && (
              <div className="mt-6 pt-6 border-t border-white/10">
                ...
                <div className="w-full h-3 bg-white/10 rounded-full overflow-hidden">
                  <div className="h-full bg-linear-to-r from-blue-500 to-cyan-500 rounded-full" style={{ width: `${progressPct}%` }} />
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex space-x-2 border-b border-white/10">
            {['overview', 'modules', 'tasks', 'team'].map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={`px-4 py-2 rounded-t-lg text-sm font-medium transition-all capitalize ${activeTab === tab ? 'bg-white/10 text-white border border-white/20 border-b-0' : 'text-slate-400 hover:text-white'}`}>
                {tab}
                ...
                {tab === 'team'    && <span className="ml-2 text-xs bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded-full">{project.teamMembers?.length || 0}</span>}
              </button>
            ))}
          </div>
          ...
          {activeTab === 'overview' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-3">
                    {tasks.slice(0, 5).map((task, i) => (
                      <div key={i} className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/10">
                        ...
                        <span className={`text-xs px-2 py-1 rounded-full ${taskStatusColor(task.status)}`}>{task.status?.replace(/_/g, ' ')}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="backdrop-blur-xl bg-white/10 rounded-2xl p-6 border border-white/20">
                ...
                {modules.length === 0 ? <p className="text-slate-400 text-sm">No modules yet</p> : (
                  <div className="space-y-3">
                    {modules.map((mod, i) => (
                      <div key={i} className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/10">
                        <div>
                        ...
                        <span className={`text-xs px-2 py-1 rounded-full ${statusColor(mod.status)}`}>{mod.status}</span>
                      </div>
                    ))}
                  </div>
                )}
            ...
            <div className="space-y-4">
              <div className="flex justify-end">
                <button onClick={() => setShowAddModule(true)}
                  className="px-4 py-2 bg-linear-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white rounded-xl text-sm font-medium flex items-center gap-2">
                  ...
                  {modules.map((mod, i) => (
                    <div key={i} className="backdrop-blur-xl bg-white/10 rounded-2xl p-6 border border-white/20 hover:bg-white/15 transition-all">
                      ...
                      <div className="space-y-1 text-xs text-slate-400">
                        <p>Tasks: <span className="text-white">{tasks.filter(t => t.module?._id === mod._id).length}</span></p>
                        <p>Completed: <span className="text-green-400">{tasks.filter(t => t.module?._id === mod._id && t.status === 'completed').length}</span></p>
                      </div>
                    </div>
                  ))}
                </div>
                    ...
                    <tbody>
                      {tasks.map((task, i) => (
                        <tr key={i} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                          ...
                          <td className="p-4"><span className={`text-xs px-2 py-1 rounded-full ${taskStatusColor(task.status)}`}>{task.status?.replace(/_/g, ' ')}</span></td>
                          <td className="p-4"><span className={`text-xs px-2 py-1 rounded-full ${priorityColor(task.priority)}`}>{task.priority}</span></td>
                          <td className="p-4 text-slate-400 text-sm">{task.dueDate ? new Date(task.dueDate).toLocaleDateString() : '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                    ...
                    <div key={i} className="backdrop-blur-xl bg-white/10 rounded-2xl p-6 border border-white/20 hover:bg-white/15 transition-all">
                      <div className="flex items-center space-x-4 mb-4">
                        ...
                        <div>
                          ...
                          <span className="text-xs px-2 py-0.5 rounded-full bg-blue-500/20 text-blue-400 capitalize">{member.role?.replace('_', ' ')}</span>
                        </div>
                      </div>
      ...
      {showAddModule && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="backdrop-blur-xl bg-slate-900/95 border border-white/20 rounded-2xl p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-slate-300 text-sm mb-1 block">Module Name *</label>
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={handleAddModule} disabled={submitting || !moduleForm.name}
                  className="flex-1 py-3 bg-linear-to-r from-blue-500 to-cyan-500 text-white rounded-xl text-sm font-medium disabled:opacity-50">
      ...
      {showAddTask && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="backdrop-blur-xl bg-slate-900/95 border border-white/20 rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-slate-300 text-sm mb-1 block">Task Title *</label>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="backdrop-blur-xl bg-slate-900/95 border border-white/20 rounded-2xl p-6 w-full max-w-md">
            ...
            <input value={memberSearch} onChange={e => setMemberSearch(e.target.value)}
              ...
              className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-white text-sm placeholder-slate-500 focus:outline-none focus:border-blue-500 mb-4" />
            ...
            <button onClick={() => setShowAddMember(false)}
              className="w-full mt-4 py-2 bg-white/5 text-slate-300 border border-white/10 rounded-xl text-sm">
              Done
            </button>
          </div>
        </div>
