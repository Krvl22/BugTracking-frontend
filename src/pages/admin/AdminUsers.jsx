...
const AdminUsers = () => {
  ...
  const handleBlock = async (e, id) => {
    e.stopPropagation();
    try {
      await fetch(`http://localhost:3000/users/${id}/block`, { method: 'PATCH', headers });
      fetchUsers();
  ...
  const handleReactivate = async (e, id) => {
    e.stopPropagation();
    try {
      await fetch(`http://localhost:3000/users/${id}/reactivate`, { method: 'PATCH', headers });
      fetchUsers();
    } catch (err) { console.error(err); }
  };
  ...
  const statusColor = (status) => {
    ...
    if (status === 'deleted') return 'bg-gray-500/20 text-gray-400';
          ...
          <div className="absolute bottom-4 left-3 right-3">
            <div className="backdrop-blur-sm bg-white/5 rounded-lg p-3 border border-white/10">
              <div className="flex items-center space-x-3">
                ...
                <div className="flex-1"><p className="text-white text-sm font-medium">Admin User</p><p className="text-slate-400 text-xs">admin@company.com</p></div>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-slate-300 text-sm">{users.length} users — click any row to view details</p>
                </div>
              </div>
              <div className="flex items-center space-x-4">
                <button onClick={() => setShowAddModal(true)} className="px-4 py-2 bg-linear-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white rounded-lg transition-all duration-200 text-sm font-medium">+ Add New User</button>
                <button onClick={handleLogout} className="px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg transition-all duration-200 text-sm font-medium">Logout</button>
              </div>
            </div>
          </div>
        </header>
            ...
            <table className="w-full">
              ...
              <tbody>
                {users.map((user, index) => (
                  <tr key={index} onClick={() => navigate(`/admin/users/${user._id}`)} className="border-b border-white/5 hover:bg-white/10 transition-colors cursor-pointer">
                    <td className="p-4">
                      <div className="flex items-center space-x-3">
                    ...
                    <td className="p-4">
                      <div className="flex items-center space-x-2">
                        {user.status === 'active' ? (
                          <button onClick={(e) => handleBlock(e, user._id)} className="px-3 py-1 bg-red-500/20 hover:bg-red-500/30 text-red-400 rounded-lg text-xs transition-colors">Block</button>
                        ) : user.status === 'blocked' ? (
                          <button onClick={(e) => handleReactivate(e, user._id)} className="px-3 py-1 bg-green-500/20 hover:bg-green-500/30 text-green-400 rounded-lg text-xs transition-colors">Reactivate</button>
                        ) : null}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="backdrop-blur-xl bg-slate-900/90 border border-white/20 rounded-2xl p-6 w-full max-w-md mx-4">
            ...
            <div className="flex space-x-3 mt-6">
              <button onClick={handleAddUser} className="flex-1 py-3 bg-linear-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white rounded-xl font-medium transition-all">Add User</button>
