...
// ── TaskFormFields — top-level so React never remounts it on parent re-render ──
const TaskFormFields = ({ form, onChange, errorMsg, modules, developers, testers }) => (
  <div className="space-y-4">
    {errorMsg && <p className="text-red-400 text-sm px-3 py-2 bg-red-500/10 border border-red-500/20 rounded-lg">{errorMsg}</p>}
    <div>
      <label className="text-slate-400 text-xs mb-1 block">Title <span className="text-red-400">*</span></label>
...
// ── MultiTaskFormList — top-level for same reason ──────────────────────────
const MultiTaskFormList = ({ forms, setForms, globalErrorMsg, modules, developers, testers }) => (
  <div className="space-y-6">
    {forms.map((form, index) => (
      <div key={index} className="relative">
        {forms.length > 1 && (
      ...
      {showCreateTask && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="backdrop-blur-xl bg-slate-900/95 border border-white/20 rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <MultiTaskFormList forms={taskForms} setForms={setTaskForms} globalErrorMsg={taskMsg} modules={modules} developers={developers} testers={testers} />
      ...
      {showCreateSprint && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="backdrop-blur-xl bg-slate-900/95 border border-white/20 rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <MultiTaskFormList forms={sprintTasks} setForms={setSprintTasks} globalErrorMsg="" modules={modules} developers={developers} testers={testers} />
      ...
      {showAddTaskToSprint && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="backdrop-blur-xl bg-slate-900/95 border border-white/20 rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <MultiTaskFormList forms={addSprintTaskForms} setForms={setAddSprintTaskForms} globalErrorMsg={addSprintTaskMsg} modules={modules} developers={developers} testers={testers} />
